export function initOilCalculator(root = document) {
  const scope = root.querySelector ? root : document;
  const form = scope.querySelector('form');
  const resultCards = scope.querySelectorAll(
    '#compound-summary, #stack-summary, #injection-plan, #monthly-calendar, #vial-duration'
  );
  const compoundCardsContainer = scope.querySelector('#compound-cards');
  const compoundSummaryList = scope.querySelector('#compound-summary-list');
  const injectionPlanCards = scope.querySelector('#injection-plan-cards');
  const monthlyCalendarCards = scope.querySelector('#monthly-calendar-cards');
  const vialDurationCards = scope.querySelector('#vial-duration-cards');
  const calendarModal = scope.querySelector('#calendar-modal');
  const calendarModalBackdrop = scope.querySelector('#calendar-modal-backdrop');
  const calendarModalClose = scope.querySelector('#calendar-modal-close');
  const calendarModalTitle = scope.querySelector('#calendar-modal-title');
  const calendarModalDate = scope.querySelector('#calendar-modal-date');
  const calendarModalBody = scope.querySelector('#calendar-modal-body');
  const calendarModalCompleted = scope.querySelector('#calendar-modal-completed');
  const calendarModalStatusLabel = scope.querySelector('#calendar-modal-status-label');
  const calendarTooltip = scope.querySelector('#calendar-tooltip');
  const addCompoundButton = scope.querySelector('#add-compound');
  const frequencySelect = scope.querySelector('#injection-frequency');
  const startDateInput = scope.querySelector('#start-date');
  const cycleNameSelect = scope.querySelector('#cycle-name');
  const cycleLengthSelect = scope.querySelector('#cycle-length');
  const protocolGoalSelect = scope.querySelector('#protocol-goal');
  const cycleNameCustomField = scope.querySelector('#cycle-name-custom-field');
  const cycleLengthCustomField = scope.querySelector('#cycle-length-custom-field');
  const goalCustomField = scope.querySelector('#goal-custom-field');
  const athleteSummaryFields = {
    athleteName: scope.querySelector('#summary-athlete-name'),
    coachName: scope.querySelector('#summary-coach-name'),
    goal: scope.querySelector('#summary-goal'),
    cycleLength: scope.querySelector('#summary-cycle-length'),
    startDate: scope.querySelector('#summary-start-date'),
    expectedFinish: scope.querySelector('#summary-expected-finish'),
  };
  ...
  const resultFields = {
    totalWeeklyDose: scope.querySelector('#result-total-weekly-dose'),
    totalWeeklyOil: scope.querySelector('#result-total-weekly-oil'),
    injectionFrequency: scope.querySelector('#result-injection-frequency'),
    totalOilPerInjection: scope.querySelector('#result-total-oil-per-injection'),
  };
  ...
  function getCycleNameLabel() {
    if (cycleNameSelect.value === 'custom') {
      const customName = scope.querySelector('#cycle-name-custom').value.trim();
  ...
  function getCycleWeeks() {
    if (cycleLengthSelect.value === 'custom') {
      const customWeeks = parseInt(scope.querySelector('#cycle-length-custom').value, 10);
  ...
  function updateAthleteSummary() {
    const athleteName = scope.querySelector('#athlete-name').value.trim();
    const coachName = scope.querySelector('#coach-name').value.trim() || 'TeamMF';
    const startDate = parseInputDate(startDateInput.value);
    const cycleWeeks = getCycleWeeks();
  ...
  function refreshCalendarIfReady() {
    if (calendarPlanData) {
      ...
      renderMonthlyCalendar(calendarPlanData);
      scope.querySelector('#monthly-calendar').hidden = false;
    }
  }
  ...
  function styleRemoveButton(button) {
    if (REMOVE_BUTTON_STYLE) {
      button.style.cssText = REMOVE_BUTTON_STYLE;
    }
  }

  function setActiveTab(tabId) {
    const tabs = scope.querySelectorAll('.calc-tab');
    const panels = scope.querySelectorAll('.calc-tab-panel');
  ...
  function applyDefaultInjectionDays(frequency) {
    ...
    DAYS.forEach(function (day) {
      scope.querySelector('#day-' + day.id).checked = defaults.indexOf(day.id) !== -1;
    });
  }

  function getSelectedInjectionDays() {
    return DAYS.filter(function (day) {
      return scope.querySelector('#day-' + day.id).checked;
    });
  }
  ...
  scope.querySelectorAll('.calc-tab').forEach(function (tab) {
  ...
  scope.querySelector('#athlete-protocol').addEventListener('input', updateAthleteSummary);
  scope.querySelector('#athlete-protocol').addEventListener('change', function (event) {
    toggleAthleteCustomFields();
    updateAthleteSummary();
  ...
  form.addEventListener('submit', function (event) {
    ...
    const syringeSize = scope.querySelector('#syringe-size').value;
    ...
    scope.querySelector('#vial-duration').hidden = vialDurationCards.children.length === 0;

    const resultsEmpty = scope.querySelector('#results-empty');
    if (resultsEmpty) {
  ...
  return function cleanup() {
    ...
    window.removeEventListener('scroll', hideCalendarTooltip, true);
    hideCalendarTooltip();
    closeCalendarModal();
  };
}
